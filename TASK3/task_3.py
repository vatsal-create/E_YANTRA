...
import math  # It was given in piazza that we can
...
previous_input_x, previous_input_y = 0, 0
previous_error_x, previous_error_y = 0, 0
previous_time = time.time()
prev_angle_x, prev_angle_y = 0, 0
PID_P_x, PID_P_y, PID_I_x, PID_I_y, PID_D_x, PID_D_y = 0, 0, 0, 0, 0, 0
kp_x, ki_x, kd_x = 25,20,60  #16, -0.52, 28
kp_y, ki_y, kd_y = 25,20,60  #16, 0.3, 28
cnt = 0
PID_Tx_maxValue, PID_Ty_maxValue = 0, 0
PID_Tx_minValue, PID_Ty_minValue = 0, 0
...
def init_setup(rec_client_id):
	...
	"""
    global client_id, vision_sensor_handle, rev_joint_handle_x, rev_joint_handle_y, ball
    ...
    return_code, rev_joint_handle_y = sim.simxGetObjectHandle(client_id, 'revolute_joint_ss_1',
                                                              sim.simx_opmode_blocking)
    return_code, rev_joint_handle_x = sim.simxGetObjectHandle(client_id, 'revolute_joint_ss_2',
                                                              sim.simx_opmode_blocking)
    return_code, ball = sim.simxGetObjectHandle(client_id, 'green_ball_1', sim.simx_opmode_blocking)
...
def control_logic(center_x, center_y):
	...
	"""
	global setpoint, client_id, previous_error_x, previous_error_y, previous_time, PID_P_x, PID_P_y, cnt, sampleTime, \
		PID_I_x, PID_I_y, PID_D_x, PID_D_y, previous_input_x, previous_input_y, kp_x, kp_y, ki_x, ki_y, kd_x, kd_y, PID_Tx_maxValue, PID_Ty_maxValue, \
		PID_Tx_minValue, PID_Ty_minValue, prev_angle_x, prev_angle_y
	...
	return_code, velocity, _ = sim.simxGetObjectVelocity(client_id, ball, sim.simx_opmode_streaming)

	dt = (initTime - previous_time)
	offset_x = velocity[0] * 2000 * 0.27
	offset_y = velocity[1] * 2000 * 0.27
	...
	rate_change_error_x = (error_x - previous_error_x) / dt
	rate_change_error_y = (error_y - previous_error_y) / dt

	lim = 2000
	if PID_I_x + ki_x * error_x * dt >= lim or PID_I_x + ki_x * error_x * dt <= -lim:
		PID_I_x = PID_I_x
	else:
		PID_I_x = PID_I_x + ki_x * error_x * dt

	if PID_I_y + ki_y * error_y * dt >= lim or PID_I_y + ki_y * error_y * dt <= -lim:
		PID_I_y = PID_I_y
	else:
		PID_I_y = PID_I_y - ki_y * error_y * dt

	PID_P_x = kp_x * error_x
	PID_P_y = -kp_y * error_y
	PID_D_x = kd_x * rate_change_error_x
	PID_D_y = -kd_y * rate_change_error_y

	PID_T_x = PID_P_x + PID_D_x + PID_I_x
	PID_T_y = PID_P_y + PID_D_y + PID_I_y

	PID_Tx_maxValue = max(PID_Tx_maxValue, PID_T_x)
	PID_Tx_minValue = min(PID_Tx_minValue, PID_T_x)
	PID_Ty_maxValue = max(PID_Ty_maxValue, PID_T_y)
	PID_Ty_minValue = min(PID_Ty_minValue, PID_T_y)

	maxValue_x = 15000
	maxValue_y = 15000
	angle_x = PID_T_x / maxValue_x * math.pi / 2
	#  angle_x = prev_angle_x + (dt / kd_x) * (angle_x - prev_angle_x)
	angle_y = PID_T_y / maxValue_y * math.pi / 2
	#  angle_y = prev_angle_y + (dt / kd_y) * (angle_y - prev_angle_y)

	tmp2 = 2
	if angle_x > math.pi / tmp2:
		angle_x = math.pi / tmp2
	if angle_x < -math.pi / tmp2:
		angle_x = -math.pi / tmp2

	if angle_y > math.pi / tmp2:
		angle_y = math.pi / tmp2
	if angle_y < -math.pi / tmp2:
		angle_y = -math.pi / tmp2

	'''print("FOR X:")
	print("Error of x: ", error_x, " PID_P_x: ", PID_P_x)
	print("Rate of change of error_x: ", rate_change_error_x, " PID_D_x: ", PID_D_x)
	print("PID_I_x: ", PID_I_x)
	print("PID_T_x: ", PID_T_x, " Angle_x: ", angle_x)
	print("PID_Tx_max: ", PID_Tx_maxValue)
	print("PID_Tx_min: ", PID_Tx_minValue)
	print("\nFOR Y:")
	print("Error of y: ", error_y, " PID_P_y: ", PID_P_y)
	print("Rate of change of error_y: ", rate_change_error_y, " PID_D_y: ", PID_D_y)
	print("PID_I_y: ", PID_I_y)
	print("PID_T_y: ", PID_T_y, " Angle_y: ", angle_y)
	print("PID_Ty_max: ", PID_Ty_maxValue)
	print("PID_Ty_min: ", PID_Ty_minValue)
	print("\nValue of dt: ", dt)
	print("\nVelocity: ", velocity)
	print("\nOffset X: ", offset_x, " Offset Y: ", offset_y)'''

	sim.simxPauseCommunication(client_id, True)
	_ = sim.simxSetJointTargetPosition(client_id, rev_joint_handle_y, angle_y, sim.simx_opmode_oneshot)
	_ = sim.simxSetJointTargetPosition(client_id, rev_joint_handle_x, angle_x, sim.simx_opmode_oneshot)
	sim.simxPauseCommunication(client_id, False)

	previous_input_x = center_x - offset_x
	previous_input_y = center_y - offset_y
	previous_error_x = error_x
	previous_error_y = error_y
	prev_angle_x = angle_x
	prev_angle_y = angle_y
	cnt += 1
	previous_time = initTime
	#print("Time taken by control logic: ", time.time() - initTime)
...
if __name__ == "__main__":
    ...
    return_code_signal, init_simulation_time_string = sim.simxGetStringSignal(client_id, 'time',
    ...
    while (curr_simulation_time - init_simulation_time <= 15):
        ...
        return_code_signal, curr_simulation_time_string = sim.simxGetStringSignal(client_id, 'time',
        ...
        try:
            ...
            if ((return_code == sim.simx_return_ok) and (len(image_resolution) == 2) and (
                    len(vision_sensor_image) > 0)):
