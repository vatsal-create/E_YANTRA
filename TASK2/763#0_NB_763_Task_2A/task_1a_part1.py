...
def scan_image(img):
    ...
    for i in reversed(range(0, len(contours) - 1)):
        ...
        if len(approx) == 3:
            if shapes.get("Triangle") == None:
                shapes["Triangle"] = [cl, cx, cy]

            shapes['Triangle'].append([cl, cx, cy])
        elif len(approx) == 4:
            shp = quad(approx)

            if shapes.get(shp) == None:
                shapes[shp] = []

            shapes[shp].append([cl, cx, cy])
        elif len(approx) == 5:
            if shapes.get("Pentagon") == None:
                shapes["Pentagon"] = []

            shapes['Pentagon'].append([cl, cx, cy])
        elif len(approx) == 6:
            if shapes.get("Hexagon") == None:
                shapes["Hexagon"] = []

            shapes['Hexagon'].append([cl, cx, cy])
        else:
            if shapes.get("Circle") == None:
                shapes["Circle"] = []

            shapes['Circle'].append([cl, cx, cy])

    ##################################################

    for key in shapes.keys():
        sort_tuple = sorted(shapes[key], key=lambda item: item[0])
        shapes[key] = sort_tuple

        if (len(shapes[key]) == 1):
            shapes[key] = shapes[key][0]
