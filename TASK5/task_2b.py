...
def get_vision_sensor_image():
    ...
    return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, vision_handle, 0,
...
def send_data(client_id, maze_array,top_plate):
    ...
    for row in maze_array:
        return_code, outInts, outFloat, outString, outBuffer = sim.simxCallScriptFunction(client_id, top_plate , sim.sim_scripttype_customizationscript,
                                                                                   'receiveData', row, [], [], emptybuff, sim.simx_opmode_blocking)
