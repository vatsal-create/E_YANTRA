...
try:
    ...
    print('sim.py, simConst.py and appropriate library - remoteApi.dll (if on Windows), remoteApi.so (if on Linux) or '
          'remoteApi.dylib (if on Mac).\n')
    sys.exit()
...
def get_vision_sensor_image(vision_sensor_handle):
    ...
    return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, vision_sensor_handle,
...
def exit_remote_api_server(client_id):
    ...
    _ = sim.simxSetJointTargetPosition(client_id, rev_joint_handle_y, 0, sim.simx_opmode_oneshot)
    _ = sim.simxSetJointTargetPosition(client_id, rev_joint_handle_x, 0, sim.simx_opmode_oneshot)
                ...
                if (type(transformed_image) is np.ndarray):
                    ...
                    try:
                        ...
                        if (type(warped_img) is np.ndarray):
                            ...
                            try:
                                shapes = task_1a_part1.scan_image(warped_img) #first get the no. of outputs
