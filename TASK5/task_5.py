...
def main(rec_client_id):
    ...
    ##############    ADD YOUR CODE HERE    ##############
    print(rec_client_id)
    maze_t4 = cv2.imread('C:\\Users\\Desktop\\E YANTRA\\TASK5\\maze_t4.jpg')
    maze_t1 = cv2.imread('C:\\Users\\Desktop\\E YANTRA\\TASK5\\maze_t1.jpg')
    warped_img_t4 = task_1b.applyPerspectiveTransform(rec_client_id,maze_t4)
    '''cv2.imshow('m4',maze_t4)
    cv2.waitKey(0)
    cv2.destroyAllWindows()'''
    warped_img_t1 = task_1b.applyPerspectiveTransform(rec_client_id,maze_t1)

    if type(warped_img_t4) is np.ndarray:
        maze_array_t4 = task_1b.detectMaze(warped_img_t4)
    if (type(maze_array_t4) is list) and (len(maze_array_t4) == 10):
        print(maze_array_t4)
    if type(warped_img_t1) is np.ndarray:
        maze_array_t1 = task_1b.detectMaze(warped_img_t1)
    if (type(maze_array_t1) is list) and (len(maze_array_t1) == 10):
        print(maze_array_t1)
    return_code=-1
    return_code = task_2b.send_data(rec_client_id,maze_array_t4,'top4')
    print(return_code)
    return_code=-1
    return_code = task_2b.send_data(rec_client_id,maze_array_t1,'top1')
    print(return_code)
    return_code = task_2a.start_simulation(rec_client_id)
    return_code = task_2a.stop_simulation(rec_client_id)
    task_2a.exit_remote_api_server(rec_client_id)
